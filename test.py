...
if __name__ == '__main__':
    ...
    sns.lineplot(x=year_counts.index, y=year_counts.values, marker='o', color='blue', label = 'count')
    plt.xlabel('Year')
    plt.ylabel('Count')
    ...
    flat_applicant = [item for sublist in data_input['applicants'].dropna() for item in sublist]
    ...
    def get_cpc_impact(cpc_list , data) :
        ...
        for cpc in cpc_set :
            ...
            index_list = [idx for idx, val in enumerate(cpc_list) if cpc in val]
    ...
    def get_cpc_CAGR(cpc_list , data) :  
        ...
        for cpc in cpc_set :
            ...
            temp_data_2000 = temp_data.loc[temp_data['year_application'] == 2008, :].reset_index(drop =1)
            temp_data_2019 = temp_data.loc[temp_data['year_application'] == 2016, :].reset_index(drop =1)
            
            cpc_CAGR_dict[cpc] = CAGR(len(temp_data_2000), len(temp_data_2019), 8)
    ...
    title = LDA_0.get_most_similar_doc2topic(data_input, title = 'title', date = 'year_application')
    ...
    sns.scatterplot(data = input_df, x= 'volume_recent', y= 'CAGR_recent', marker = 'o')
